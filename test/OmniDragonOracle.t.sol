...
contract OmniDragonOracleTest is Test {
    ...
    function setUp() public {
        ...
        oracle = new OmniDragonOracle(
            ...
            delegate,
            owner,
            OmniDragonOracle.OracleMode.PRIMARY
        );
    ...
    function testDeployment() public {
        ...
        assertEq(uint8(oracle.mode()), uint8(OmniDragonOracle.OracleMode.PRIMARY));
        assertEq(oracle.priceInitialized(), false);
    ...
    function testConfigureOracles() public {
        ...
        oracle.configureOracles(
            ...
            address(0), // No Band feed
            address(0), // No API3 feed  
            address(0), // No Pyth feed
            bytes32(0), // No Pyth ID
            "S" // Band symbol
        );
        
        (address feedAddress, uint256 weight, bool isActive, uint256 maxStaleness) = 
            oracle.oracleConfigs("chainlink");
        
        assertEq(feedAddress, address(chainlinkFeed));
        assertEq(weight, 3000); // Default weight from _initializeDefaultConfigs
        assertEq(isActive, true);
        assertEq(maxStaleness, 3600); // Default staleness
    }
    
    function testConfigureDragonSonicPair() public {
        vm.prank(owner);
        oracle.configureDragonSonicPair(
