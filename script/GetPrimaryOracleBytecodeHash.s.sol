...
contract GetPrimaryOracleBytecodeHash is Script {
    ...
    // Sonic endpoint (from registry):
    address constant SONIC_ENDPOINT = 0x6F475642a6e85809B1c36Fa62763669b1b48DD5B;

    function run() external view {
        bytes memory initCode = abi.encodePacked(
            type(OmniDragonPrimaryOracle).creationCode,
            abi.encode("NATIVE", "USD", OWNER, REGISTRY, DRAGON, SONIC_ENDPOINT, DELEGATE)
        );
        bytes32 bytecodeHash = keccak256(initCode);
        console.log("PRIMARY_ORACLE BYTECODE HASH:");
